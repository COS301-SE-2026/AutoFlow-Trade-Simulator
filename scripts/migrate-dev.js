...
const msg = process.argv.slice(2).join(' ') || `auto migration ${new Date().toISOString().replace(/[:.]/g,'')}`;
...
console.log('Ensuring database is at current head');
let res = spawnSync(alembic, ['-c', 'alembic.ini', 'upgrade', 'head'], { cwd: backendDir, stdio: 'inherit', shell: false });
if (res.status !== 0) process.exit(res.status);

console.log('Generating migration:', msg);
res = spawnSync(alembic, ['revision', '--autogenerate', '-m', msg], { cwd: backendDir, stdio: 'inherit', shell: false });
if (res.status !== 0) process.exit(res.status);
